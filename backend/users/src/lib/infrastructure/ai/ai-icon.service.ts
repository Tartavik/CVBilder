...
export class AiIconService {
  ...
  constructor(private readonly configService: ConfigService) {
    ...
    this.rateLimitWindowMs = Number(
      this.configService.get<number>('AI_ICON_RATE_LIMIT_WINDOW_MS') ?? 60_000,
    );
    this.maxRequestsPerWindow = Number(
      this.configService.get<number>('AI_ICON_RATE_LIMIT_MAX') ?? 5,
    );
    this.maxGenerationsPerUser = Number(
      this.configService.get<number>('AI_ICON_MAX_GENERATIONS_PER_USER') ?? 10,
    );
    this.mockOpenAi =
      this.configService.get<string>('AI_ICON_MOCK_MODE') === 'true' ||
      this.configService.get<string>('CI') === 'true';
  }

  async findOrGenerateIcon(
    userId: string,
    skillName: string,
  ): Promise<SkillIconResult> {
  ...
  private async generateWithOpenAI(skillName: string): Promise<string> {
    ...
    try {
      ...
      const imageBase64 = (
        response as { data?: Array<{ b64_json?: string; url?: string }> }
      ).data?.[0]?.b64_json;
      if (imageBase64) {
  ...
  private ensureGenerationAllowed(userId: string): void {
    ...
    if (recentRequests.length >= this.maxRequestsPerWindow) {
      throw new HttpException(
        'Too many icon requests. Please try again shortly.',
    ...
    if (state.generatedCount >= this.maxGenerationsPerUser) {
      throw new HttpException(
        'Generation limit reached for this user.',
        HttpStatus.TOO_MANY_REQUESTS,
      );
    }
